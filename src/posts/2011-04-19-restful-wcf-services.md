...
WCF not only provides SOAP, it’s also capable of providing RESTful services through use of the WebGet and WebInvoke attributes in the System.ServiceModel.Web assembly in conjunction with the webHttpBinding and webHttp behaviour. In fact, WebGet/WebInvoke are not really required (however PUT is the default Http Verb and you won’t be able to specify what methods correspond to urls).
...
![](/images/blog/image_thumb_38.png)

As REST support only arrived in WCF 3.5 there is still a lack of support in certain areas, e.g. error handling, caching etc; what I’ve seen done in practice is people continue to use the REST Starter kit from codeplex.
